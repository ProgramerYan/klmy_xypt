		...
		function loadDataByYear(year){		
			//load_SingleValues('2c9eeb506407c8f301640c086f980004',year);
		}
		//清除共享方式内容
	...
	　　});
		...
		function alert_show(name)
		{
			$.get(getJsonUrl(table_BMZT_GXFS_DataSourceId,name)).done(function (data) {
					var id=data.toString();//.rows[0]["VALUE"];
					update_Right_Table_Info(name);
					//alert(id);
			});
