	...
	//载入本页面初始数据
		...
		function loadChart_1(year)
		{
		  var chartID="HYFW_QXT";
				...
				//加载曲线图数据
		function loadChart_2(year)
		{
		  var chartID="HYFW_QXT_SJJH";
				$.get(getChartJsonUrl(chartID,year)).done(function (data) {		
					var Labels=new Array(); //存放标签的数组
					var Values=new Array();//存放数据对象的数组
					for (var i=0;i<data.rows.length;i++){
				...
				//加载曲线图数据
		function loadChart_3(year)
		{
		  var chartID="HYFW_QXT_SJZL";
				$.get(getChartJsonUrl(chartID,year)).done(function (data) {		
					var Labels=new Array(); //存放标签的数组
					var Values=new Array();//存放数据对象的数组
					for (var i=0;i<data.rows.length;i++){
		...
		//初始化接入单位
		function initChart_JRDW(valueObj)
		{
			var aTitle = ['累计', '本周', '本月', '本季'];
			var data = [valueObj.LJRK_NUM,valueObj.BZRK_NUM,valueObj.BYRK_NUM,valueObj.BJRK_NUM];
			judgeStr_HRDW(aTitle, data, 7, 4, 'top-center');

			function judgeStr_HRDW(aTitle, data, numMaxLength, blockNum, parentClassName) {
			...
			function createTitle(blockNum, aTitle, aBlock) {
				for (var i = 0; i < blockNum; i++) {
					...
					borderDiv.classList.add('first-border-div');
					describe.classList.add('first-describe');
			...
			function crateNumber(blockNum, numMaxLength, data, aBlock, aspan) {
				for (var h = 0; h < blockNum; h++) {
					...
					for (var j = 0; j < numMaxLength; j++) {
						...
						aspan.push(oSpan);
						oNumber.classList.add('first-number');
						oSpan.classList.add('first-number-span');
